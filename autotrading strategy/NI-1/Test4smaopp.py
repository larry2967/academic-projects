# -*- coding: utf-8 -*-
"""
Created on Sun Apr  1 19:49:04 2018
...
def Short_run_ema(filename, n, data):
    ...
    ema = moving_average(data['C'], n, type='simple')
    return ema 
...
def Trading_Sig(filename, n, m, data):
  ...
  #  print(len(sig))
    ...
    for i in range(m-1, ns):
        if data['C'][i] < shorEma[i]:
            sig[i] = 1
            #print(sig[i])
        elif data['C'][i] > shorEma[i]:
            sig[i] = -1
            #print(sig[i])
        else:
            sig[i] = 0
    ...
    comp = pd.DataFrame({"Price Closing": data['C'], "SEMA": shorEma, "Signal": sig})
...
def calcAll (m,n):
    totalprofit = []
    for i in range (len(filenames)):
        totalp = gross_profits (filenames[i], m, n)
